...
- (void)showCellWithKJAssetCollection:(KJAssetCollection *)assetCollection {
    if (assetCollection.assetCollection.assetCollectionSubtype == PHAssetCollectionSubtypeSmartAlbumUserLibrary) {
        _titleLabel.text = [NSString stringWithFormat:@"相机胶卷（%ld）",(long)assetCollection.count];
    }else {
        _titleLabel.text = [NSString stringWithFormat:@"%@（%ld）",assetCollection.assetCollection.localizedTitle,(long)assetCollection.count];;
    }
